...
async function getLastRuns(): Promise<Record<string, LastRun>> {
  ...
  if (error) {
    console.error('[CGI] could not read agent_runs:', error.message)
    return {}
  }
...
function outcomeClass(outcome: string | null): string {
  ...
  if (o === 'ok' || o === 'done' || o === 'executed') return 'good'
  if (o === 'failed' || o === 'error') return 'bad'
  if (o === 'escalated' || o === 'noop') return 'warn'
  return 'info'
}
...
export default async function Employees() {
  ...
  return (
    <>
      ...
      <p className="cap">Your robot team — each one&apos;s job and how much it&apos;s allowed to do on its own. The dial is severity: below your line it acts and tells you, at or above it (and anything overdue) it asks first.</p>
      {AGENTS.map(a => {
          ...
          <div className="agent-card" key={a.key}>
            <p className="ac-name">
              ...
              {run ? (
                ...
                <span className="pill muted">not run yet</span>
              )}
            </p>
            ...
            <p className="ac-lastrun">
              {run
                ...
                : 'Last run: — (this robot hasn’t run yet)'}
            </p>
          </div>
        )
      })}
