...
// One line in the "needs you" lists.
function Item({ r, today }: { r: Rec; today: string }) {
  const info = (CATEGORY_INFO as any)[r.category]
  return (
    <Link href={info?.href || '/'} className="item">
...
export default async function Dashboard() {
  ...
  return (
    <>
      ...
      {overdue.length ? (
        <div className="items">{overdue.slice(0, 6).map(r => <Item key={r.id} r={r} today={today} />)}</div>
      ) : (
      ...
      {soon.length ? (
        <div className="items">{soon.slice(0, 5).map(r => <Item key={r.id} r={r} today={today} />)}</div>
      ) : (
      ...
      {high.length ? (
        <div className="items">{high.slice(0, 5).map(r => <Item key={r.id} r={r} today={today} />)}</div>
      ) : (
      ...
      <table className="tbl">
        ...
        <tbody>
          {registers.map(([cat, b]) => {
            ...
            return (
              <tr key={cat}>
                <td data-label="Register"><Link href={info?.href || '/'}>{info?.emoji} {catPlural(cat)}</Link></td>
                <td data-label="Items" className="r">{b.total}</td>
                <td data-label="Open" className="r">{b.open}</td>
                <td data-label="Overdue" className="r">{b.overdue ? <span className="pill bad">{b.overdue}</span> : '—'}</td>
                <td data-label="Due 7d" className="r">{b.dueSoon ? <span className="pill warn">{b.dueSoon}</span> : '—'}</td>
                <td data-label="High+" className="r">{b.high || '—'}</td>
              </tr>
            )
          })}
        </tbody>
